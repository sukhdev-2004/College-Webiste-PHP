...
$select_stud = mysqli_query($conn, "SELECT * FROM `stud` WHERE `email` = '$id'");
$row = mysqli_fetch_assoc($select_stud);
$course = $row['course'];

$select_teacher = mysqli_query($conn, "SELECT * FROM `teacher` WHERE `department` = '$course' AND `status` = 'Accept'");
...
<body>
    <h1>Teachers in <?php echo $course; ?> Department</h1>
    <div class="container">
        <?php
        if ($tot_teacher > 0) {
